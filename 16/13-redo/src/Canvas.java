...
public class Canvas extends JPanel {
    ...
    public Canvas(File filename) {
        ...
        nextShape = 0;              // Start with no shapes
    ...
    public Canvas(BufferedReader br, String fileVersion) throws IOException {
        ...
        while(size-- > 0) {
            ...
            if      (shapeID.equals(Polyline.id()))  addShape(new Polyline(br, fileVersion));
            else if (shapeID.equals(Text.id()))      addShape(new Text(br, fileVersion));
            else if (shapeID.equals(Image.id()))     addShape(new Image(br, fileVersion));
            else throw new IOException("Unknown shape ID: " + shapeID);

/*
            Fails because id() is not a "constant String expression"  :-(  
            switch(shapeID) {
                case Polyline.id() -> addShaped(new Polyline(br, fileVersion));
                case Text.id()     -> addShape(new Text(br, fileVersion));
                case Image.id()    -> addShape(new Image(br, fileVersion));
...
*/
    ...
    public void save(BufferedWriter bw) throws IOException {
        ...
        for(int i=0; i<nextShape; ++i)
            shapes.get(i).save(bw);      // Tell each active shape to save itself
        ...
        if ((event.getButton() == MouseEvent.BUTTON1) &&
            ...
            if(!clickInProgress) { // first click
                ...
                if(penMode == PenMode.TEXT) {
                    addShape(new Text(text, x1, y1, color, font));
                } else if(penMode == PenMode.IMAGE) {
                    if(image != null)
                        addShape(new Image(imagename, x1, y1, this, image));
                } else {
    ...
    public void paintComponent(Graphics graphics) {
        ...
        // Paint all shapes in the ArrayList up to nextShape
        for(int i=0; i<nextShape; ++i)
            shapes.get(i).paintComponent(g);
    ...
    protected int nextShape;      // For redo
    ...
    protected void clearRedoBuffer() {
        if(nextShape < shapes.size()) // If any shapes are not in use
            shapes.subList(nextShape, shapes.size()).clear(); // clear them
    }
    public void undo() {
        if(nextShape > 0) {--nextShape; repaint();}  // Mark latest shape as unused
    }
    public void redo() {
        if(shapes.size() > nextShape) {++nextShape; repaint();} // If available, restore one
    }
