...
public class Sudoku {
    ...
    private static String layout =
      ...
      + "┃ . . . │ . . . │ . . . ┃\n"
      + "┗━━━━━━━┷━━━━━━━┷━━━━━━━┛\n";
    private static String dotSubs = ".123456789";
    
    @Override
    public String toString() {
        StringBuilder result = new StringBuilder(layout);

        int nextDot = result.indexOf(".");
        for(int y=0; y<9; ++y) {
            for(int x=0; x<9; ++x) {
                int c = board[x][y];
                result.replace(nextDot, nextDot+1, 
                       dotSubs.substring(c, c+1));
                nextDot = result.indexOf(".", nextDot+1);
            }
        }
        return name + "\n" + "-".repeat(name.length()) + "\n" + result.toString();
    }
