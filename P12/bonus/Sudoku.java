...
public class Sudoku {
    ...
    public boolean solve(int depth, String path) {
        for(int x=0; x<9; ++x) {
            for(int y=0; y<9; ++y) {
                if(board[x][y] == 0) {
                    //if(depth < 18) System.out.println(path);
                    for(int v=1; v<=9; ++v) {
                        //System.out.println("Checking " + v + " at (" + x + "," + y + ")");
                        if(isSafe(x, y, v)) {
                            //System.out.print("  It's safe!");
                            board[x][y] = v;
    ...
    private boolean isSafe(int x, int y, int v) {
        for(int i=0; i<9; ++i) { //System.out.print(" "+x+i +" "+i+y);
        ...
        for(int i=minX; i<minX+3; ++i) {
            for(int j=minY; j<minY+3; ++j) { //System.out.print(" "+i+j);
