...
public class Question {
    ...
    public String toString() {
        // Shuffle answers and reset rightAnswer to the same answer
        // If using an ArrayList, consider Collections.shuffle and indexOfSubList!
        String rightAnswerString = answers[rightAnswer-1];
        String temp;
        for(int i=0; i<answers.length; ++i) {
            int left = random.nextInt(answers.length);
            int right = random.nextInt(answers.length);
            temp = answers[left];
            answers[left] = answers[right];
            answers[right] = temp;
        }
        for(int i=0; i<answers.length; ++i) {
            if(answers[i] == rightAnswerString) {
                rightAnswer = i+1;
                break;
            }
        }
    ...
    private int rightAnswer;  // From 1 to number of proposed answers (answers.length)
